...
public class MusicDirViewModel extends BaseViewModel{

    private static final String TAG = "MusicDirViewModel";
    ...
    private SoftReference<MusicDirViewModel.EditInputChangeListener> mTextListener;
    private SoftReference<MusicDirAdapter> adapter;

    public MusicDirViewModel(Application application) {
        super(application);
        mApplication = application;
        mTextListener = new SoftReference<>(new MusicDirViewModel.EditInputChangeListener());
        phoneRefresh = 60;
    }
    ...
    public void setAdapter(MusicDirAdapter adapter) {
        clearAdapter();
        this.adapter = new SoftReference<>(adapter);
    }

    public MusicDirViewModel.EditInputChangeListener getTextListener(){
...
//            adapter.get().searchMediaItems(newText);
    ...
    public MusicDirViewModel.MyCheckedListener getCheckedListener(){
        return new SoftReference<>(new MusicDirViewModel.MyCheckedListener()).get();
    }
    private class MyCheckedListener implements CompoundButton.OnCheckedChangeListener{
        ...
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
            ...
            MusicDirViewModel.this.setCustomStyle(isChecked);
    ...
    public MusicDirViewModel.BarTimerTask getCircleBarTask(){
        return getSoftReference(new MusicDirViewModel.BarTimerTask());
    }
            ...
            if (playbackState.getState() == PlaybackStateCompat.STATE_PLAYING) {//音乐正在播放
                ...
                /*Log.d(TAG, "run: 当前音乐播放进度 "+position+ "  音乐时长："+
                        mMediaControllerCompat.getMetadata()
                                .getLong(MediaMetadataCompat.METADATA_KEY_DURATION));*/
                //mMusicDirViewModel.setMax((int) duration);
